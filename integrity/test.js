...
async function main() {
  ...
  manifestServer.listen(MANIFEST_PORT, '127.0.0.1', async () => {
    console.log('Manifest server is listening.');
    ...
    try {
      ...
      console.log('Forging update token using public.pem as HS256 secret...');
      ...
      const payload = {
        repoUrl: `http://127.0.0.1:${MANIFEST_PORT}/manifest.json`,
      ...
      cleanup(0, server, manifestServer);
    } catch (err) {
      console.error('TEST FAILED:', err);
      cleanup(1, server, manifestServer);
    }
  });
}

function cleanup(code, server, manifestServer) {
  try {
    if (manifestServer) {
    ...
    console.error('Error while shutting down servers:', e.message);
  } finally {
    process.exit(code);
  }
}
